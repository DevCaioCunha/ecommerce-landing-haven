...
export const SupplierSidebar = ({ open }: SupplierSidebarProps) => {
  ...
  const menuItems = [
    ...
    { icon: ShoppingCart, name: "Pedidos", path: "/supplier-orders" },
    { icon: Truck, name: "Entregas", path: "/supplier-deliveries" },
    { icon: FileText, name: "Relatórios", path: "/supplier-reports" },
    { icon: Calendar, name: "Calendário", path: "/supplier-calendar" },
    { icon: Settings, name: "Configurações", path: "/supplier-settings" },
  ];

  return (
    <aside 
      className={`fixed left-0 top-[65px] z-9 h-[calc(100vh-65px)] bg-white shadow-sm transition-all duration-300 ${
